> Historical implementation notes through 2026-09-20. Status statements below describe their original batch, not current support. See [the current plan](implementation-plan.md) and [README](../README.md).
...
### 第十八批：粘贴对话框，以及拆掉那道协议闸门

进度（2026-09-20）：已完成。

- `app::proxy_import`（新）：只带一个“分享链接”输入框的对话框。它只负责解析，写入仍由
  `AppState` 做，与 `ProxyEditor` 同构：解析被拒时对话框不关，且显示的是解析器自己那句话。
  名字取链接自带的 remark（vmess 的 `ps`，其余取 fragment），没有就用“协议 + host”。
- Proxies 页新增 "Import from link"；四种没有表单的协议从此有了入口。
- **拆掉 `ProxyService` 的第二道闸门**：它原来拒绝“构建器不支持的协议”，而构建器现在六种都支持，
  这道闸门的含义已经消失。`runtime::is_supported` 一并删除——它把两个问题混成了一个
  （“配置能不能建”与“表单能不能填”），现在前者是构建器的事实，后者是表单自己的常量
  （`ProxyKind::ALL` 与 `ProxyKind::BY_LINK`）。
- 编辑器的说明文字随之改了：不再说“运行时还没有构建器”，而是“这四种不在这里填，用链接导入”。
- 测试：application 侧“导入的协议与手填的一样能存”；编辑器侧“六个协议都有入口
  （两个表单 + 四个链接）”；UI 侧“粘贴真链接 → 行出现且名字来自 remark”、
  “读不了的链接说明原因、不建任何东西、对话框不关”。
